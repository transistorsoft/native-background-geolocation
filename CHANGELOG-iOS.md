...
- feat(WO-006): pause location updates when the sentinel says the device has stopped. NOTE: as shipped in 4.5.0 this suspended every registered location stream — `watchPosition` included — and cancelled every in-flight single-shot request, not only the polygon hit-tester as this line originally implied. Both are corrected in 4.5.2 (WO-018).
